...
class Paragraph {
    ...
    constructor({ data, config, api, readOnly }) {
         ...
         * @type {string}
         */
        this._placeholder = this.config.placeholder ?? Paragraph.DEFAULT_PLACEHOLDER;

        this._data = {
            text: (data && data.text) || '',
            alignment: (data && data.alignment) || this.config.defaultAlignment || Paragraph.DEFAULT_ALIGNMENT,
        };
     ...
     */
    merge(data) {
        const newText = (this.data.text || '') + (data.text || '');
    ...
    renderSettings() {
        ...
        this.settings
            .map((tune) => {
                ...
                // Используем стандартный класс Editor.js для кнопок настроек:
